...
// Multer config to store uploads in client/public/uploads
...
const upload = multer({ storage: storageEngine, limits: { fileSize: 5 * 1024 * 1024 } }); // 5MB limit

export async function registerRoutes(app: Express): Promise<Server> {
  // Auth middleware
  await setupAuth(app);

  // Simple upload endpoint for ID images
  app.post("/api/uploads/id-image", isAuthenticated, upload.single("idImage"), async (req: any, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ message: "No file uploaded" });
      }
      // Build public URL relative to client/public
  ...
  app.put('/api/products/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!existingProduct || existingProduct.farmer_id !== farmerId && existingProduct.farmerId !== farmerId) {
        // support both snake_case and camelCase depending on storage
  ...
  app.get('/api/orders/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Verify access
      if (order.buyer_id !== userId && order.buyerId !== userId && order.farmer_id !== userId && order.farmerId !== userId) {
        return res.status(403).json({ message: "Unauthorized" });
      }
  ...
  app.put('/api/orders/:id/status', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!existingOrder || (existingOrder.farmer_id !== userId && existingOrder.farmerId !== userId)) {
        return res.status(403).json({ message: "Unauthorized" });
      }
  ...
  app.put('/api/profile/verify/:id/review', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (updated && status === "approved") {
        await storage.upsertUser({ id: updated.user_id ?? updated.userId, isVerified: true });
      }
